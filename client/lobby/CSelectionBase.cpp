...
void TownSelector::updateListItems()
{
	...
	for (auto const & factionID : CGI->townh->getDefaultAllowed())
	{
		if(y >= line && (y - line) < 3)
		{
			...
			townsArea[factionID] = std::make_shared<ClickableArea>(Point(48, 32), [this, getImageIndex, factionID](const Point & cursorPosition){
				townsEnabled[factionID] = !townsEnabled[factionID];
				towns[factionID]->setFrame(getImageIndex(factionID, townsEnabled[factionID]));
				redraw();
			}, [factionID](const Point & cursorPosition){ CRClickPopup::createAndPush(factionID.toFaction()->town->faction->getNameTranslated()); });
			townsArea[factionID]->moveTo(pos.topLeft() + Point(x_offset + 48 * x, 32 * (y - line)));
		}
