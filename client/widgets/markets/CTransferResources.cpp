...
void CTransferResources::makeDeal()
{
	if(auto toTrade = offerSlider->getValue(); toTrade != 0)
	{
		...
		LOCPLINT->cb->trade(market, EMarketMode::RESOURCE_PLAYER, GameResID(bidTradePanel->getHighlightedItemId().value()),
			PlayerColor(offerTradePanel->getHighlightedItemId().value()), toTrade, hero);
...
CMarketBase::MarketShowcasesParams CTransferResources::getShowcasesParams() const
{
	if(bidTradePanel->isHighlighted() && offerTradePanel->isHighlighted())
		...
		{
			ShowcaseParams {std::to_string(offerSlider->getValue()), bidTradePanel->getHighlightedItemId().value()},
			ShowcaseParams {CGI->generaltexth->capColors[offerTradePanel->getHighlightedItemId().value()], offerTradePanel->getHighlightedItemId().value()}
		};
	else
...
void CTransferResources::highlightingChanged()
{
	if(bidTradePanel->isHighlighted() && offerTradePanel->isHighlighted())
	{
		offerSlider->setAmount(LOCPLINT->cb->getResourceAmount(GameResID(bidTradePanel->getHighlightedItemId().value())));
...
std::string CTransferResources::getTraderText()
{
	if(bidTradePanel->isHighlighted() && offerTradePanel->isHighlighted())
	{
		...
		message.replaceName(PlayerColor(offerTradePanel->getHighlightedItemId().value()));
