...
{
	if(ArtifactID(offerTradePanel->getHighlightedItemId().value()).toArtifact()->canBePutAt(hero))
	{
		assert(offerTradePanel->getHighlightedItemId());
		LOCPLINT->cb->trade(market, EMarketMode::RESOURCE_ARTIFACT, GameResID(bidTradePanel->getHighlightedItemId().value()),
			ArtifactID(offerTradePanel->getHighlightedItemId().value()), offerQty, hero);
...
CMarketBase::MarketShowcasesParams CArtifactsBuying::getShowcasesParams() const
{
	if(bidTradePanel->isHighlighted() && offerTradePanel->isHighlighted())
		...
		{
			ShowcaseParams {std::to_string(deal->isBlocked() ? 0 : bidQty), bidTradePanel->getHighlightedItemId().value()},
			ShowcaseParams {std::to_string(deal->isBlocked() ? 0 : offerQty), CGI->artifacts()->getByIndex(offerTradePanel->getHighlightedItemId().value())->getIconIndex()}
		};
	else
...
void CArtifactsBuying::highlightingChanged()
{
	if(bidTradePanel->isHighlighted() && offerTradePanel->isHighlighted())
	{
		market->getOffer(bidTradePanel->getHighlightedItemId().value(), offerTradePanel->getHighlightedItemId().value(),
			bidQty, offerQty, EMarketMode::RESOURCE_ARTIFACT);
		deal->block(LOCPLINT->cb->getResourceAmount(GameResID(bidTradePanel->getHighlightedItemId().value())) < bidQty || !LOCPLINT->makingTurn);
	}
...
std::string CArtifactsBuying::getTraderText()
{
	if(bidTradePanel->isHighlighted() && offerTradePanel->isHighlighted())
	{
		...
		message.replaceName(ArtifactID(offerTradePanel->getHighlightedItemId().value()));
		message.replaceNumber(bidQty);
		message.replaceTextID(bidQty == 1 ? "core.genrltxt.161" : "core.genrltxt.160");
		message.replaceName(GameResID(bidTradePanel->getHighlightedItemId().value()));
